...
- (id)initWithTBXML:(TBXMLElement*)aXMLElement
{
	if ((self = [super init]))
	{
		if (aXMLElement)
		{
			...
			//Refresh Cache
			if(newCurrencyName && ![newCurrencyName isEqualToString:@""] && ![[newCurrencyName lowercaseString] isEqualToString:@"nil"] )
			{
...
- (void)updateWithTBXML:(TBXMLElement*)aXMLElement shouldCheckEarnedPoints:(BOOL)checkEarnedPoints
{
	...
	BOOL tapPointsUpdated = false;
	
	if(tapPoints_ != newTapPoints)
	{
		tapPointsUpdated = true;
	}
	...
	if(tapPointsUpdated)
		[[NSNotificationCenter defaultCenter] postNotificationName:TJC_TAP_POINTS_UPDATED object:nil];
}
